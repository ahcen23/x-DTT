function [bitsPerPixel,bpp,SizeCoded, jpeg_decoded] = jpegIDTT(image, qf)
...
for i=1:blocsFiles
    for j=1:blocsColumnes
        ...
        %blocDct = dct2(bloc);
        [~,dtt_int,dtt_diag]=DTT(8);
        blocDct=(dtt_diag^-1*dtt_int)*bloc*(dtt_diag^-1*dtt_int)';
        %blocDct=dtt_kernel*bloc*dtt_kernel';
...
for i=1:blocsFiles
    ...
    bloc = bloc.*(G);
    bloc=((dtt_diag^-1*dtt_int)'*bloc*(dtt_diag^-1*dtt_int))+128;
    %bloc=round((dtt_kernel'*bloc*dtt_kernel)+128);
